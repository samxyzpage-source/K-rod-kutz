...
test('nfl division: seeded coin when everything is equal — deterministic per rng, both outcomes reachable', () => {
  ...
  for (let seed = 1; seed <= 40; seed++) {
    ...
    deq(r1, r2, 'same rng seed → same standings');
  ...
  deq(St.compute(s.season, s.league), St.compute(s.season, s.league));
});
...
test('nfl wild card: division winners seeded 1–4 by record, wild cards 5–7, sweep rule and conference record', () => {
  ...
  // Sweep: s2 beat both n2 and w2 (all pairs played: n2 beat w2, so no sweep without s2's).
  fx.h2h(s.season, s2, n2, { conf: true }); fx.h2h(s.season, s2, w2, { conf: true }); fx.h2h(s.season, n2, w2, { conf: true });
  ...
  assert.equal(seed(s2), 5, 'head-to-head sweep takes the first wild card (despite the worst conference record)');
  assert.equal(seed(n2), 6, 'the remaining two restart: n2 beat w2 head-to-head (over w2\'s better conference record)');
  assert.equal(seed(w2), 7);
  assert.equal(rows.find((r) => r.teamId === s2).note, 'head-to-head sweep');
  assert.equal(rows.find((r) => r.teamId === n2).note, 'head-to-head sweep');
  // Without the n2–w2 game the sweep step is skipped ("if all play") and conference record decides.
  delete s.season.results[n2].h2h[w2]; delete s.season.results[w2].h2h[n2];
  const rows2 = St.compute(s.season, s.league, leagueFx.testRng(RTG, 1));
  assert.equal(rows2.find((r) => r.teamId === w2).seed, 5, 'conference record 8-4');
  assert.equal(rows2.find((r) => r.teamId === w2).note, 'conference record');
  assert.equal(rows2.find((r) => r.teamId === s2).seed, 6, 'the two left restart: s2 beat n2 head-to-head');
  assert.equal(rows2.find((r) => r.teamId === n2).seed, 7);
  assert.equal(rows.filter((r) => r.conf === 'Liberty' && r.seed).length, 7);
  assert.equal(rows.filter((r) => r.conf === 'Frontier' && r.seed).length, 7);
  deq(rows.filter((r) => r.conf === 'Liberty').map((r) => r.confRank), Array.from({ length: 16 }, (_, i) => i + 1));
});
...
test('recordResult updates both rows (REG counts conf/div marks, postseason does not)', () => {
  ...
  deq([ra.w, ra.l, ra.t, ra.pf, ra.pa, ra.divW, ra.divL, ra.confW, ra.confL], [1, 1, 1, 54, 50, 1, 0, 1, 0]);
  deq([rb.w, rb.l, rb.divW, rb.divL, rb.confW, rb.confL], [1, 1, 0, 1, 0, 1]);
  deq(ra.h2h[b], [1, 1, 0]); deq(rb.h2h[a], [1, 1, 0]); deq(ra.h2h[c], [0, 0, 1]);
...
test('college rankings: §2.6.1 formula, preseason uses prestige/5, weekly score is sticky', () => {
  ...
  deq(ranks, Array.from({ length: 48 }, (_, i) => i + 1), 'ranks are a permutation of 1..48');
...
test('playoffField: 5 champions auto-qualify, 7 at-large by rank, the 6th champion is eligible as an at-large', () => {
  ...
  deq(b.seeds.slice(0, 4).map((x) => x.teamId), champIds.slice(0, 4), 'seeds 1–4 = four highest-ranked champions');
  deq(b.byes, champIds.slice(0, 4));
...
test('advanceBracket (college): fixed bracket 1v(8/9) … 4v(5/12), bowl venues, champion crowned, exit rounds', () => {
  ...
  deq(qf.map((m) => [m.homeSeed, m.awaySeed]), [[1, 9], [2, 10], [3, 11], [4, 12]], 'QF pairings with all upsets');
  ...
  deq(b.rounds[2].games.map((m) => [m.homeSeed, m.awaySeed]), [[1, 4], [2, 3]], 'semifinals');
  St.advanceBracket(b, play(), s.league);
  deq(b.rounds[3].games.map((m) => [m.homeSeed, m.awaySeed]), [[1, 2]]);
...
test('bowls: ≥ 6 wins, not in the playoff, paired across conferences into the minor bowls, week 14', () => {
  ...
  for (let seed = 1; seed <= 20; seed++) {
    ...
    deq(St.bowls(s.season, s.league, s.rng), bowls);
  }
...
test('full college postseason via fixtures produces a champion and validates', () => {
  ...
  deq(JSON.parse(JSON.stringify(s.season)), s.season);
});
...
test('nflPlayoffField + advanceBracket: 7 per conference, seed 1 bye, 2v7/3v6/4v5, re-seeding, Championship Bowl', () => {
  ...
  for (const conf of ['Liberty', 'Frontier']) {
    ...
    deq(cs.map((x) => x.seed), [1, 2, 3, 4, 5, 6, 7]);
    ...
    deq(wc.map((m) => [m.homeSeed, m.awaySeed]), [[2, 7], [3, 6], [4, 5]]);
    assert.ok(wc.every((m) => m.venue.type === 'HOME' && m.venue.hostTeamId === m.homeId));
  }
  deq(b.rounds.map((r) => [r.name, r.week, r.kind]), [['WC', 19, 'WC'], ['DIV', 20, 'DIV'], ['CONF', 21, 'CONF'], ['CHAMP', 22, 'CHAMP']]);
  ...
  for (const conf of ['Liberty', 'Frontier']) {
    ...
    deq(dv.map((m) => [m.homeSeed, m.awaySeed]), [[1, 7], [4, 6]], `${conf} divisional round re-seeded`);
  ...
  for (const conf of ['Liberty', 'Frontier']) deq(b.rounds[2].games.filter((m) => m.conf === conf).map((m) => [m.homeSeed, m.awaySeed]), [[1, 4]]);
  St.advanceBracket(b, fav(), s.league);
  ...
  deq(JSON.parse(JSON.stringify(b)), b);
});
...
test('draftOrder: worst record first (point differential, then coin), playoff teams by exit round, champion last', () => {
  ...
  deq(St.draftOrder(s.season, s.league, leagueFx.testRng(RTG, 1)), order);
  s.season.playoffs = null;
...
test('compute rows are complete, JSON-safe and grouped by conference', () => {
  ...
  deq(JSON.parse(JSON.stringify(rows)), rows);
  for (const r of rows) {
  ...
  deq(rows.map((r) => r.rank).sort((x, y) => x - y), Array.from({ length: 32 }, (_, i) => i + 1));
  deq(St.rowsIn(rows, 'Liberty').map((r) => r.confRank), Array.from({ length: 16 }, (_, i) => i + 1));
  deq(St.rowsIn(rows, 'Liberty', 'North').map((r) => r.divRank), [1, 2, 3, 4]);
