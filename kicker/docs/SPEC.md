...
| Potential | `POT` (hidden, per attribute) | Hard cap per attribute. `POT[a] ~ clamp(round(N(80, 8)), 60, 99)`; 5-star recruit `+6`, 4-star `+3`, walk-on `−3`. Revealed to the player by agent tier (§2.7.10). **The archetype's signature attribute has no cap** (D24): `Tuning.progression.signature` names it (CANNON → POW, SURGEON → ACC, ICEMAN → CLU, SOCCER → KO), `Player.create` pins its POT at 99 after the draws (so the draw order is unchanged) and `applyStars` never moves it. |
...
**Scale.** Catalogue prices are $k × `Finance.scale(state)` = `Tuning.finance.scale[league]`, and the scale is
**1 in both leagues** (D28): the $25k truck is $25k in college and in the NFL, the lake house $500k — the pros simply
earn more, so the books go from "what can I afford" to "what do I do with it". The knob stays in Tuning (the first
money version priced the NFL ×10, which read as inflation); the league is `player.league`, falling back to the stage.
...
2. **Charge the lifestyle plan** at the price **quoted when the books last closed** — `finance.planCost`, which
   ...
   its quoted price at the first NFL tick whatever the scale then is; the next close re-quotes); with no quote (fixtures, a
...
5. **Debt**: if the bank is below **the grace floor** (`−Tuning.finance.debt.grace × scale`, $10k — a small
   overdraft from event costs before any income is not a debt year) — interest
...
| Tier | Cost / yr (both leagues) | Every year | The line |
|---|---|---|---|
| `FRUGAL` (default) | $0 | — | Roommates, a beater and rice. The leg does not care where you sleep. |
| `COMFORTABLE` | $12k | morale +2 | Your own place, a car that starts, a proper bed. Nothing to post about. |
| `FLASHY` | $40k | morale +4, fame +15, fans +2 | The good building, the good table, the good watch. People notice. |
| `BALLER` | $110k | morale +6, fame +40, fans +4, **trust −3** | Private chef, a driver, a jet card. Everyone notices — the coach included. |
...
`Tuning.finance.timeline.minDelta × scale` ($25k) makes the timeline). They are lifestyle only — no purchase
...
| `scale(state)` | `Tuning.finance.scale[league]` (1 in both leagues since D28) | 0 | pure |
...
1. **THE BOOKS · Y<year>** (`.fin-head`, gold — red in debt): BANK (`.fin-bank`), NET WORTH (`.fin-networth`),
   TAKE-HOME Y<year> (this year's `INCOME` rows), the lifestyle chip, a `PRO PRICES ×n` chip only when the scale is above 1 (never, since D28), the
