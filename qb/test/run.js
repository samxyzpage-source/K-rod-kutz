#!/usr/bin/env node
/**
 * Test runner: runs every qb/test/*.test.js in its own Node process.
 *
 *   node qb/test/run.js             # fast suite (skips files tagged [balance])
 *   node qb/test/run.js --balance   # everything, including slow statistical tests
 *   node qb/test/run.js play rng    # only files whose name contains one of the args
 *
 ...
 * is also runnable directly:  node qb/test/play.test.js
 */
'use strict';
const fs = require('fs');
