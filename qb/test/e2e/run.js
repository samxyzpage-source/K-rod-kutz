#!/usr/bin/env node
/**
 * Playwright e2e runner: starts the static http server, runs every qb/test/e2e/*.spec.js in its own Node
 * process (like qb/test/run.js), stops the server, exits non-zero when any spec fails.
 *
 *   /opt/node22/bin/node qb/test/e2e/run.js              # every spec
 *   /opt/node22/bin/node qb/test/e2e/run.js boot moment  # only specs whose name contains one of the args
 *   RTG_PORT=8081 node qb/test/e2e/run.js                # another port
 *
 * Each spec uses node:test + node:assert/strict and is also runnable directly: node qb/test/e2e/boot.spec.js
 ...
 *
 * NEVER edit source while a run is in progress (false failures).
 */
'use strict';
const fs = require('fs');
...
(async () => {
  ...
  console.log('e2e/run.js: http://127.0.0.1:' + srv.port + '/qb/ · ' + files.length + ' spec(s)');
  let failed = 0;
