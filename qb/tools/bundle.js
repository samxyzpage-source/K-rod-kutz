#!/usr/bin/env node
/**
 * bundle.js — builds a single self-contained HTML file from qb/index.html by
 ...
 *   node qb/tools/bundle.js --fragment out.html  # body-only fragment (no doctype/html/head/body tags)
 *
...
const ROOT = path.resolve(__dirname, '..');   // qb/
const args = process.argv.slice(2);
const fragment = args.includes('--fragment');
const outArg = args.filter((a) => !a.startsWith('--'))[0];
const OUT = outArg ? path.resolve(outArg) : path.join(ROOT, 'dist', fragment ? 'qb.fragment.html' : 'qb.html');
